...
**In scope:** entities, fields, and relationships needed to run the system's core flows.

**Out of scope** (handled by other skills): user interface, APIs, analytics and reporting, workflows and automations, role and permission design beyond naming which entities need view/edit permissions, and infrastructure choices.

## 3. Naming convention

{{One short paragraph stating which convention was chosen:

- **If template:** "Entity and field names follow the {{Vendor}} canonical schema, lowercased to snake_case. This keeps import/export with {{Vendor}} straightforward."
- **If agent-optimised:** "Entity and field names are self-describing and snake_case. Names avoid abbreviations so an agent can reason about the model without vendor-specific priors."}}
...
_(repeat section 5 per entity, numbered 5.1, 5.2, …)_

## 6. Relationship summary

A single table showing every link between entities. An agent uses this to sanity-check that each reference field in §5 has a corresponding row here.
...
## 9. Implementation notes for the downstream agent
...
2. Create entities in the order given in §4 — entities referenced by others first.
3. For each entity: set `label_column` to the snake_case field marked as label in §5, pass `module_id`, `view_permission`, `edit_permission`. Do **not** manually create `id`, `created_at`, `updated_at`, or the auto-label field.
4. For each field in §5: pass `table_name`, `field_name`, `format`, `title` (the Label column), `is_nullable` (inverse of Required), and for `reference`/`parent` fields also `reference_table` and a `reference_delete_mode` consistent with §6.
...
- If the system has no enums, §7 can read "No enumerations defined." — don't omit the section; keeping section numbers stable helps humans navigate multiple PRDs.
